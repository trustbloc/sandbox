...
import (
	...
	"github.com/trustbloc/edge-sandbox/test/bdd/restclient"
)
...
// FileHandlerSteps include steps
...
// NewFileHandlerSteps return new file handler steps
...
func (d *FileHandlerSteps) createDocument(url, content, namespace string) error {
	...
	doc, err := d.getOpaqueDocument(content)
	if err != nil {
		return err
	}

	req, err := getCreateRequest(doc)
	if err != nil {
		return err
	}
...
func (d *FileHandlerSteps) getOpaqueDocument(content string) (string, error) {
...
func (d *FileHandlerSteps) RegisterSteps(s *godog.Suite) {
	s.Step(`^client sends request to "([^"]*)" to create document with content "([^"]*)" in namespace "([^"]*)"$`,
		d.createDocument)
	s.Step(`^client sends request to "([^"]*)" to retrieve file$`, d.resolveFile)
	s.Step(`^client sends request to "([^"]*)" to upload file "([^"]*)" with content type "([^"]*)"$`,
		d.uploadFile)
