...
do
   ...
   --insecure http://strapi:1337/admin/auth/local/register | jq -r '.jwt')
   echo "token: $token"
   if [ -n "$token" ]
   then
     break
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   ...
   --insecure http://strapi:1337/users | jq  -r ".error")
# check for error
if [ "$result" != "null" ]
   then
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   ...
   --insecure http://strapi:1337/studentcards | jq  -r ".error")
# check for error
if [ "$result" != "null" ]
   then
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   ...
   --insecure http://strapi:1337/transcripts | jq  -r ".error")
# check for error
if [ "$result" != "null" ]
   then
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   ...
   --insecure http://strapi:1337/travelcards | jq  -r ".error")
# check for error
if [ "$result" != "null" ]
   then
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   ...
   --insecure http://strapi:1337/permanentresidentcards | jq  -r ".error")
# check for error
if [ "$result" != "null" ]
   then
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   ...
   --insecure http://strapi:1337/universitydegreecredentials | jq  -r ".error")
# check for error
if [ "$result" != "null" ]
   then
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   ...
   --insecure http://strapi:1337/creditcardstatements | jq  -r ".error")
# check for error
if [ "$result" != "null" ]
   then
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   --request POST \
   --data '{"userid":"100","metadata": {"contexts":["https://trustbloc.github.io/context/vc/examples/mdl-v1.jsonld"],"scopes":["mDL"],"name":"Drivers License","description":"Drivers License for John Smith (Issued by Government of Castleham)"},"data":{"given_name": "John", "family_name": "Smith", "document_number": "123-456-789","birthdate":"1990-01-01","driving_privileges":"G2","issuing_country":"Canada","issuing_authority":"Ministry of Transport Ontario","issue_date":"2020-05-27","expiry_date":"2025-05-26","resident_address":"4726 Pine Street","resident_city":"Toronto","resident_postal_code":"A1B 2C3","resident_state":"Ontario"}}' \
   --insecure http://strapi:1337/mdls | jq  -r ".error")
# check for error
if [ "$result" != "null" ]
   then
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   --request POST \
   --data '{"userid":"100","metadata":{"contexts":["https://trustbloc.github.io/context/vc/examples/credit-score-v1.jsonld"],"scopes":["CreditScore"]},"data":{"family_name":"Smith","given_name":"John","birthdate":"1990-01-01","address":"4726 Pine Street, Toronto - A1B 2C3","report_date":"2020-08-14","score":"737"}}' \
   --insecure http://strapi:1337/creditscores | jq  -r ".error")
# check for error
if [ "$result" != "null" ]
   then
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   ...
   --insecure http://strapi:1337/mdlevidences | jq  -r ".error")
# check for error
if [ "$result" != "null" ]
   then
