...
GENERATE_STUDENTAPI_COMMAND="strapi generate:api studentcards UserID:string StudentID:string Name:string Email:string University:string Semester:string Type:string"

$GENERATE_STUDENTAPI_COMMAND

# generate the transcript api and model
GENERATE_TRANSCRIPT_COMMAND="strapi generate:api transcripts UserID:string StudentID:string Name:string University:string Course:string Status:string TotalCredits:string Type:string"

$GENERATE_TRANSCRIPT_COMMAND

# generate the travel card api and model
GENERATE_TRAVELCARD_COMMAND="strapi generate:api travelcards UserID:string TravelCardID:string Name:string Sex:string Country:string DOB:string IssueDate:string CardExpires:string Type:string"

$GENERATE_TRAVELCARD_COMMAND

# generate the pr card api and model
GENERATE_PRCARD_COMMAND="strapi generate:api prcards UserID:string Name:string FamilyName:string Gender:string Image:string ResidentSince:string LPRCategory:string LPRNumber:string BirthCountry:string BirthDate:string Type:string"

$GENERATE_PRCARD_COMMAND

# generate the certifiedmilltestreports and model
GENERATE_CMTR_COMMAND="strapi generate:api certifiedmilltestreports UserID:string Cmtr:json"

$GENERATE_CMTR_COMMAND

# generate the crudeproductcredentials and model
GENERATE_CMTR_COMMAND="strapi generate:api crudeproductcredentials UserID:string Customcontext:json Producer:string Category:string HsCode:string Identifier:string Name:string Description:string Volume:string ProductionDate:string PredecessorOf:string SuccessorOf:string Address:json PhysicalSpecs:json ChemicalSpecs:json"
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   --request POST \
   --data '{"userid":"100","studentid":"1234568","name":"Foo","email":"foo@bar.com","university":"Faber College","semester":"3","type":"StudentCard"}' \
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   --request POST \
   --data '{"userid":"100","studentid":"323456898","name":"Foo","university":"Faber College","status":"graduated","totalcredits":"100","course":"Bachelors in Computing Science","type":"Transcript"}' \
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   --request POST \
   --data '{"userid":"100","travelcardid":"123-456-765","name":"Foo","sex":"M","country":"Canada","dob":"12-06-1989","issuedate":"01-06-2018","cardexpires":"01-06-2023","type":"TravelCard"}' \
...
result=$(curl --header "Content-Type: application/json" --header "Authorization: Bearer $token" \
   --request POST \
   --data '{"userid":"100","name":"Foo","type":"PRCard","familyname":"Smith","gender":"Female","image":"data:image/png;base64,iVBORw0KGgo...kJggg==","residentsince":"2015-01-01","lprcategory":"C09","lprnumber":"01-06-2023","birthcountry":"Bahamas","birthdate":"1958-08-17"}' \
