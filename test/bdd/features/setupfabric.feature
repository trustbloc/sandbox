...
Feature:
  Scenario: setup fabric
    Given DCAS collection config "dcas-mychannel" is defined for collection "dcas" as policy="OR('Org1MSP.member','Org2MSP.member','Org3MSP.member')", requiredPeerCount=2, maxPeerCount=3, and timeToLive=60m
    Given DCAS collection config "docs-mychannel" is defined for collection "docs" as policy="OR('Org1MSP.member','Org2MSP.member','Org3MSP.member')", requiredPeerCount=2, maxPeerCount=3, and timeToLive=60m
    Given off-ledger collection config "meta_data_coll" is defined for collection "meta_data" as policy="OR('Org1MSP.member','Org2MSP.member','Org3MSP.member')", requiredPeerCount=0, maxPeerCount=0, and timeToLive=60m
    ...
    And "system" chaincode "sidetreetxn_cc" is instantiated from path "in-process" on the "mychannel" channel with args "" with endorsement policy "AND('Org1MSP.member','Org2MSP.member','Org3MSP.member')" with collection policy "dcas-mychannel"
    And "system" chaincode "document_cc" is instantiated from path "in-process" on the "mychannel" channel with args "" with endorsement policy "OR('Org1MSP.member','Org2MSP.member','Org3MSP.member')" with collection policy "docs-mychannel,meta_data_coll"

    Given DCAS collection config "consortium-files-coll" is defined for collection "consortium" as policy="OR('Org1MSP.member','Org2MSP.member','Org3MSP.member')", requiredPeerCount=2, maxPeerCount=3, and timeToLive=60m
    And "system" chaincode "files" is instantiated from path "in-process" on the "mychannel" channel with args "" with endorsement policy "OR('Org1MSP.member','Org2MSP.member','Org3MSP.member')" with collection policy "consortium-files-coll"
    ...
    And variable "org3ConfigUpdate" is assigned the JSON value '{"MspID":"Org3MSP","Peers":[{"PeerID":"peer0.org3.example.com","Apps":[{"AppName":"file-handler","Version":"1","Config":"${fileHandlerConfig}","Format":"json"}]},{"PeerID":"peer1.org3.example.com","Apps":[{"AppName":"file-handler","Version":"1","Config":"${fileHandlerConfig}","Format":"json"}]}]}'
    ...
    And fabric-cli is executed with args "ledgerconfig update --config ${org3ConfigUpdate} --noprompt"
    ...
    When client sends request to "http://localhost:48726/.well-known/did-trustbloc/peer0.org1.example.com.json" to retrieve file
    Then the retrieved file contains "payload"
