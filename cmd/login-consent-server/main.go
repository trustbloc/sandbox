...
const (
	...
	claimsConfigFilePathEnvKey = "CLAIMS_CONFIG_FILE_PATH"
...
func main() {
	...
	http.HandleFunc("/claim-data", c.claimData)
...
func (c *consentServer) claimData(w http.ResponseWriter, req *http.Request) {
	...
	claimsConfigFilePath, isSet := os.LookupEnv(claimsConfigFilePathEnvKey)
	if !isSet {
		w.WriteHeader(http.StatusInternalServerError)
		fmt.Fprint(w, fmt.Errorf("%s value is empty", claimsConfigFilePathEnvKey))

		return
	}

	f, err := os.Open(claimsConfigFilePath) //nolint:gosec // user-defined path required
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		fmt.Fprint(w, "error opening claims config file:", err.Error())
	...
	defer func() {
		err = f.Close()
		if err != nil {
			panic(fmt.Errorf("failed to close claims config file: %w", err))
		}
	}()

	var claims map[string]interface{}

	if err = json.NewDecoder(f).Decode(&claims); err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		fmt.Fprint(w, "failed to decode claims:", err.Error())
	...
	profileID := req.URL.Query().Get("profile_id")

	if claims[profileID] == nil {
		w.WriteHeader(http.StatusInternalServerError)
		fmt.Fprint(w, "claims for profile ", profileID, " are missing")
	...
	_ = json.NewEncoder(w).Encode(claims[profileID]) //nolint:errcheck
}
